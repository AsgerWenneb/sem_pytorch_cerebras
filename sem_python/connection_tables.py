...
def gen_EToV(elemsx, elemsy):  # n1 is x-axis, n2 is y-axis
    ...
    for i in range(elemsx):
        for j in range(elemsy):
            ...
            EToV[idx, :] = [j + (i+1)*(elemsy + 1), j + i*(elemsy + 1),
                            j + (i+1)*(elemsy + 1) + 1]  # Upper triangle
            EToV[idx + 1, :] = [j + i*(elemsy + 1) + 1, j + (i+1) *
                                # Lower triangle
                                (elemsy + 1) + 1, j + i*(elemsy + 1)]
    return EToV
...
def ti_connect2D(elemsx, elemsy):  # EToF is trivial, so not returned
    ...
    for i in range(elemsx):
        for j in range(elemsy):
            ...
            # EToE
            EToE[idx, :] = [idx - 1, idx + 1, idx +
                            elemsy*2 + 1]  # Upper triangle
        ...
        for i in range(Nfaces):
            ...
            if P > 1:
                if EToE[n, i] >= n:  # if neighbor has higher index than current index
                    ...
                    list_of_idx = [
                        Nfaces + (i)*(Mpf-2) + j for j in range(Mpf-2)]
                    C[n, list_of_idx] = C[kc, [
                        Nfaces + (i)*(Mpf-2) + Mpf-3 - j for j in range(Mpf-2)]]

        # Internal points
        if P > 2:
            # Param in range is amount of already numbered points
            list_of_idx = [
                Nfaces + Nfaces*(Mpf - 2) + k for k in range(Mp - Nfaces - Nfaces*(Mpf - 2))]
            C[n, list_of_idx] = [
                gidx + k for k in range(Mp - Nfaces - Nfaces*(Mpf - 2))]
